...
$roll = array('مدير على النظام','عضو مجلس إدارة','موظف');
    
$this->load->view('admin/email/main_tabs'); 
echo form_open_multipart('Emails/inbox/'.$type.'/'.$id.'',array('class'=>"myform",'id'=>'fileupload'))
?>
...
<input type="hidden" name="page_id" id="page_id" value="<?php echo $id ?>" />
                        <div class="col-sm-10 r-message-left">
                              ...
                              <?php
                                if(isset($fetch_users))
                                    foreach($fetch_users as $user) { 
                                        ...
                                        if($type == 'reply')
                                            if($result['email_from'] == $user->user_id)
                                                $selected = 'selected';
                                                
                                        $subtext = 'data-subtext=" -- '.$roll[$user->role_id_fk].' -- "';
                                        
                                        echo'<option '.$subtext.' value="'.$user->user_id.'" '.$selected.'>'.$user->user_username.'</option>';
                                    }
                              ?>
                            </select>
                            <br /><br />
                            
                            <input class="form-control" type="text" value="<?php if(isset($result)) echo $result['title'] ?>" id="title" name="title" placeholder="الموضوع  :" required />
                            ...
                            <div class="col-xs-2">
                                ...
                                <button type="button" style="height: 35px;width: 85px; padding: 0px;" id="uploadBtn" class="btn btn-success m-t-20"><i class="fa fa-envelope-o"></i> إرسال</button>
                                <br /><br /><br />
                            </div>
