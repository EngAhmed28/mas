...
class Administrative_affairs extends CI_Controller
{
    ...
    $data['metakeyword'] = 'إضافة موظف';
    $data['metadiscription'] ='إضافة موظف';
    $data['subview'] = 'admin/administrative_affairs/add_employee';
    $this->load->view('index', $data);
    }
}
   ...
   /* public function  add_permits(){
        ...
        $data['permits'] = $this->administrative_affairs_setting->select_search_key_2('permits','emp_code',$_SESSION['emp_code'],'month',date("m"));
        ...
        if($this->input->post('add')){
            $this->Permit->insert(1);
            redirect('Administrative_affairs/add_permits', 'refresh');
        }else{
            $data['title'] = '  إضافة إذن';
            $data['metakeyword'] = '  إضافة إذن';
            $data['metadiscription'] ='  إضافة إذن';
   ...
   $data['subview'] = 'admin/administrative_affairs/permits';
    ...
    public function  edit_permits($id){
        ...
        if($this->input->post('edit')){
            ...
            $data['metakeyword'] = 'تعديل إذن';
            $data['metadiscription'] ='تعديل إذن';
            $data['subview'] = 'admin/administrative_affairs/permits';
            $this->load->view('index', $data);
        }
    }
...
//=======================================================================================
    ...
    public function add_vacation(){
        ...
        $data['get_depart_name'] = $this->Employee->select_depart_name();
        $data['admin'] = $this->Employee->select_by();
        $data['employ_name'] = $this->Employee->select_employ_name();
        $data['main_depart'] = $this->Difined_model->select_search_key('department_jobs','from_id_fk',0);
        if($this->input->post('values')){
           ...
           // $this->test($_POST);
            ...
            if ( sizeof($arr)< 0) {
                ...
                $type =0 ; if($_SESSION['role_id_fk'] ==3): $type=1; else: $type=2; endif;


                $this->Vacation->insert($type);
            ...
            $data['title'] = 'إضافة أجازة';
            $data['metakeyword'] = 'إضافة أجازة';
            $data['metadiscription'] ='إضافة أجازة';
...
public function add_vacation(){
   ...
   //die;
    ...
    /*select_search_key('permits','id',$id);*/
  
  /*echo '<pre>';
  print_r($data['group_affairs_id_fk']);  
    echo '</pre>';*/
    //die;
    /*
echo '<pre>';
...
$total_holiday_remain = 0;
 ...
 if(isset($data['group_affairs_id_fk']) && !empty($data['group_affairs_id_fk']) && $data['group_affairs_id_fk']!=null){
...
echo 'total_holiday_remain = '. $total_holiday_remain;
*/
//die;
...
//=============================================
    public function update_vacation($id){
        ...
        $data['employ_name'] = $this->Employee->select_employ_name();
        $data['title'] = 'تعديل أجازة';
        $data['metakeyword'] = 'تعديل أجازة';
        $data['metadiscription'] ='تعديل أجازة';
        $data['subview'] = 'admin/administrative_affairs/add_vacation';
        $this->load->view('index', $data);
    }
//=============================================================
...
//=======================================================================//
    ...
    public function emp_attendance(){
        ...
        $data['title'] = 'الحضور و الإنصراف';
        $data['metakeyword'] =   'الحضور و الإنصراف';
        $data['metadiscription'] =  'الحضور و الإنصراف';
    ...
    public function attendance_upload(){
        ...
        $data['title'] = 'رفع ملفات الحضور و الإنصراف';
        $data['metakeyword'] =  'رفع ملفات الحضور و الإنصراف';
        $data['metadiscription'] =  'رفع ملفات الحضور و الإنصراف';
    ...
    //=========================================================================
    public function all_depart_emp(){
        $this->load->model('Penalty_model');
        $this->load->model('Difined_model');
        $data['records'] = $this->Difined_model->select_search_key('department_jobs','from_id_fk',0);
        $data['employs'] = $this->Difined_model->select_all('employees','','','','');
        $data['title'] = 'تقرير الموظفين';
        $data['metakeyword'] = 'تقرير الموظفين';
        $data['metadiscription'] ='تقرير الموظفين';
        $data['subview'] = 'admin/report_emp/all_departs_emp';
        $this->load->view('index', $data);
    }
    ...
    public  function EmployeesDebts(){ //
        ...
        $data['metakeyword'] = 'إضافة السلف';
        $data['metadiscription'] = 'إضافة السلف';
    ...
    public function UpdateEmployeesDebts($debt_id){
        ...
        $data['metakeyword'] = 'تعديل طلب السلفه';
        $data['metadiscription'] =  'تعديل طلب السلفه';
        $data['subview'] = 'admin/administrative_affairs/debts/employee_debts';
        $this->load->view('index', $data);
    }
    ...
    public function EmployeesDebtsApproved(){
        ...
        $data['metakeyword'] = 'إعتماد  طلبات السلف';
        $data['metadiscription'] = 'إعتماد  طلبات السلف';
    ...
    public function EmployeesDebtsReport(){
        ...
        if ($this->input->post('debt_date_to') && $this->input->post('debt_date_from') && $this->input->post('type')) {
            ...
            $data['metakeyword'] = 'تقرير طلبات السلف';
            $data['metadiscription'] = 'تقرير طلبات السلف';
            $data['subview'] = 'admin/administrative_affairs/debts/debts_report';
            $this->load->view('index', $data);
        }
    }
    //-----------------------------------------------
    public function all_permissions_alltime()
    {   $this->load->model('administrative_affairs_models/Permit');
        $this->load->model('Difined_model');
        $this->load->model('Penalty_model');
        $data['records'] = $this->Difined_model->select_all('permits','emp_code','','','');
        $data['get_name'] = $this->Penalty_model->select_emp_name();
        $data['title'] = 'تقرير إحصائي للأذونات';
        $data['metakeyword'] =  'تقرير إحصائي للأذونات';
        $data['metadiscription'] =  'تقرير إحصائي للأذونات';
        $data['subview'] = 'admin/reports/all_permissions_alltime';
        $this->load->view('index', $data);

    }
    //====================================
    public function checks($emp,$diff,$value){
        $this->db->select('*');
        $this->db->from('permits');
        $this->db->where('emp_code',$emp);
        $this->db->where($diff,$value);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
...
//===============================================================================================//
    ...
    public function all_permissions_period()
    {   $this->load->model('administrative_affairs_models/Permit');
        $this->load->model('Difined_model');
        $this->load->model('Penalty_model');
        $data['get_name'] = $this->Penalty_model->select_emp_name();
        if ($this->input->post('form_date') && $this->input->post('to_date') && $this->input->post('type')) {
            $data['views'] =  $this->Permit->get_details_beetween_dates(strtotime($this->input->post('form_date')),strtotime($this->input->post('to_date')),$this->input->post('type'));
            $this->load->view('admin/reports/get_permission_data',$data);
        }else {
            $data['title'] = 'تقرير الأذونات خلال فترة';
            $data['metakeyword'] =  'تقرير الأذونات خلال فترة';
            $data['metadiscription'] =  'تقرير الأذونات خلال فترة';
            $data['subview'] = 'admin/reports/all_permissions_period';
            $this->load->view('index', $data);
        }
    }
    ...
    public function all_permissions_emp()
    {
        $this->load->model('administrative_affairs_models/Permit');
        $this->load->model('Difined_model');
        $this->load->model('Penalty_model');
        $data['emp'] = $this->Difined_model->select_all('employees','','','','');
        $data['get_name'] = $this->Penalty_model->select_emp_name();
        if ($this->input->post('form_date') && $this->input->post('to_date') && $this->input->post('emp')) {
            $data['views'] =  $this->Permit->get_details_beetween_dates_emp(strtotime($this->input->post('form_date')),strtotime($this->input->post('to_date')),$this->input->post('emp'));
            $this->load->view('admin/reports/get_permission_data_byemp',$data);
        }else {
            $data['title'] = 'أذونات الموظفين';
            $data['metakeyword'] = 'أذونات الموظفين';
            $data['metadiscription'] = 'أذونات الموظفين';
            $data['subview'] = 'admin/reports/all_permissions_emp';
            $this->load->view('index', $data);
        }
    }

    //=========================================================
    public function all_vacations_period()
    {   $this->load->model('administrative_affairs_models/Vacation');
        $this->load->model('Difined_model');
        if ($this->input->post('form_date') && $this->input->post('to_date') && $this->input->post('type')) {
            $data['all_vacations']=$this->Vacation->select_all_vacations($this->input->post('type'),$this->input->post('form_date'),$this->input->post('to_date'));
            $this->load->view('admin/administrative_affairs/get_vacations_data',$data);
        }else {
            $data['title'] = 'تقرير الأجازات خلال فترة';
            $data['metakeyword'] =  'تقرير الأجازات خلال فترة';
            $data['metadiscription'] =  'تقرير الأجازات خلال فترة';
            $data['subview'] = 'admin/administrative_affairs/all_vacations_period';
            $this->load->view('index', $data);
        }
    }

    public function all_vacations_emp(){
        $this->load->model('administrative_affairs_models/Vacation');
        $this->load->model('Difined_model');
        $data['emp'] = $this->Difined_model->select_all('employees','','','','');
        if ($this->input->post('form_date') && $this->input->post('to_date') && $this->input->post('emp')) {
            $data['all_vacations'] =$this->Vacation->select_all_vacations_emp($this->input->post('emp'),$this->input->post('form_date'),$this->input->post('to_date'));
            $this->load->view('admin/administrative_affairs/get_vacations_data_byemp',$data);
        }else{
            $data['title'] = 'أجازات الموظفين';
            $data['metakeyword'] = 'أجازات الموظفين';
            $data['metadiscription'] =  'أجازات الموظفين';
            $data['subview'] = 'admin/administrative_affairs/all_vacations_emp';
            $this->load->view('index', $data);
        }
    }
      ...
      public function  add_penalty()
    {
        ...
        if($this->input->post('add')){
            ...
            $this->load->view('admin/penalty/get_departs',$data);
        }elseif ($this->input->post('valuu')) {
            $data['id']=$this->input->post('valuu');
            $this->load->view('admin/penalty/get_departs2',$data);
        }else {
            $data['title'] = 'إضافة جزاء';
            $data['metakeyword'] =  'إضافة جزاء';
            $data['metadiscription'] =  'إضافة جزاء';
            $data['subview'] = 'admin/penalty/add_penalty';
            $this->load->view('index', $data);
        }
    ...
    public function  edit_penalty($id){
        ...
        if($this->input->post()){
            ...
            $data['title'] = 'تعديل جزاء';
            $data['metakeyword'] =  'تعديل جزاء';
            $data['metadiscription'] =  'تعديل جزاء';
            $data['subview'] = 'admin/penalty/add_penalty';
            $this->load->view('index', $data);
        }
    }
    ...
    public function suspend_penalty($id){
        $this->load->model('Penalty_model');
    ...
    public function  add_awards()
{
    ...
    if($this->input->post('add')){
        $this->Awards_model->insert();
        redirect('Administrative_affairs/add_awards', 'refresh');
    }else {
        $data['title'] = 'إضافة مكأفآت';
        $data['subview'] = 'admin/administrative_affairs/add_awards';
        $this->load->view('index', $data);
    }
...
public function  edit_awards($id){
    ...
    $data['depart_name'] = $this->Awards_model->get_department_name();
    if($this->input->post()){
        $this->Awards_model->update($id);
        redirect('Administrative_affairs/add_awards', 'refresh');
    }else{
        $data['title'] = 'تعديل مكأفآت';
        $data['subview'] = 'admin/administrative_affairs/add_awards';
        $this->load->view('index', $data);
    }
}
public function delete_awards($id){
    ...
    public function salaries()
    {
        ...
        $data['salaries'] = $this->Difined_model->select_salary();
        
    
     $data['datas'] = $this->Difined_model->funcname();
    

     $data['title'] = 'مسير المرتبات';
            $data['subview'] = 'admin/reports/salary_test';
            $this->load->view('index', $data);
