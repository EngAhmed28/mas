...
onclick="myFunction(<?php echo $record->id ?>,<?php echo $x ?>);return total_sum(<?php echo $record->id ?>,<?php echo $operation->id ?>,<?php echo $operation->price ?>,<?php echo sizeof($operations[$record->dep_id])?>);" 
...
id="operation<?php echo $operation->id ?>" name="operation[<?php echo $operation->id ?>]" class="<?php echo $record->id ?>" /></td> 
     ...
     class="form-control input-sm col-xs-2 pull-right"  value="0" rel="3" step="any" readonly="readonly" />
...
function clac(max,id) {
  ...
  totalSum = 0;
          for (i = 0; i < kar; i++) { 
  per= parseFloat(  ($("#dis_per"+i+id).val()) / 100);
